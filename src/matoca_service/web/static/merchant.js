...
document.querySelector("#join-form").addEventListener("submit",async(event)=>{event.preventDefault();if(submitting)return;submitting=true;const form=event.currentTarget,answers=[...document.querySelectorAll("[data-answer]")].map((input)=>Number(input.value));setFormBusy(form,true);try{const response=await fetch(`/api/merchants/${key}/waiting`,{method:"POST",headers:{...baseHeaders,"Content-Type":"application/json"},body:JSON.stringify({shop_id:selected.id,adult_count:counts.adult,child_count:counts.child,answer1:answers[0]??0,answer2:answers[1]??null,in_advance_information:""})});if(!response.ok){const body=response.status===409?await response.json().catch(()=>({})):{};throw new Error(body.detail)}joinDialog.close();await refresh()}catch(error){document.querySelector("#join-error").textContent=error.message||"順番待ちの申し込みに失敗しました"}finally{submitting=false;setFormBusy(form,false)}});
document.querySelector("#cancel-form").addEventListener("submit",async(event)=>{event.preventDefault();if(submitting)return;submitting=true;const form=event.currentTarget;setFormBusy(form,true);try{const response=await fetch(`/api/merchants/${key}/waiting/${cancelDialog.dataset.waitingId}`,{method:"DELETE",headers:baseHeaders});if(!response.ok){const body=response.status===409?await response.json().catch(()=>({})):{};throw new Error(body.detail)}cancelDialog.close();await refresh()}catch(error){document.querySelector("#cancel-error").textContent=error.message||"順番待ちの取消に失敗しました"}finally{submitting=false;setFormBusy(form,false)}});
setInterval(()=>{if(!document.hidden)refresh()},60000);
async function refreshWaiting(){if(!data)return;try{const response=await fetch(`/api/merchants/${key}/waiting`,{headers:baseHeaders});if(response.ok){data.waiting=await response.json();renderQueue();renderShops()}}catch{}}
